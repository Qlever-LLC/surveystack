...
const defaults = createCommonjsModule((module) => {
...
const defaults_1 = defaults.defaults;
const defaults_2 = defaults.getDefaults;
const defaults_3 = defaults.changeDefaults;
 ...
 * Renderer
 */
const Renderer_1 = class Renderer {
  constructor(options) {
 ...
 */
const InlineLexer_1 = class InlineLexer {
  ...
  output(src) {
    ...
    while (src) {
      ...
      if (cap = this.rules.em.exec(src)) {
        src = src.substring(cap[0].length);
        out += this.renderer.em(this.output(cap[6] || cap[5] || cap[4] || cap[3] || cap[2]
          || cap[1]));
        continue;
      }
      ...
      if (cap = this.rules.text.exec(src)) {
        src = src.substring(cap[0].length);
        if (this.inRawBlock) {
          out += this.renderer.text(this.options.sanitize ? (this.options.sanitizer ? this
            .options.sanitizer(cap[0]) : escape$3(cap[0])) : cap[0]);
        } else {
 ...
 */
const TextRenderer_1 = class TextRenderer {
  ...
  tok() {
    ...
    switch (this.token.type) {
      ...
      case 'list_item_start': {
        body = '';
        const { loose } = this.token;
        const { checked } = this.token;
        const { task } = this.token;
