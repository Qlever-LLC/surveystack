...
import { createReq, createRes, createSurvey } from '../testUtils';
...
describe('submissionController', () => {
  describe('getSubmissionsCsv', () => {
    it('returns expected CSV for geojson question type', async () => {
      ...
      const submission = await createSubmission({}, 'location');
      console.log(1111, survey, submission);
      const mockReq = createReq({ query: { showCsvMeta: 'true' } });
      const mockRes = await createRes();
      await getSubmissionsCsv(mockReq, mockRes);

      const expected = submission.headers;
